@echo off
REM =============================================================
REM Seed SCIM metadata for AD groups in WSO2 IS H2 database
REM
REM DYNAMIC: Discovers all groups in OU=Grupos from Samba AD
REM via ldapsearch, then seeds SCIM metadata for each one.
REM Can be re-run after adding/removing groups in AD.
REM
...
REM Samba AD connection
set SAMBA_CONTAINER=samba-ad
set DOMAIN=TSTF
set ID_BASE=1001

echo =============================================
echo  Seeding SCIM metadata for AD groups
echo  (dynamic discovery from Samba AD)
echo =============================================
echo.

REM Wait for WSO2 IS to be ready (ensures DB schema exists and groups discovered from AD)
...
REM Discover groups from Samba AD
echo.
echo ^>^> Discovering groups in OU=Grupos...

REM Query Samba AD and extract group names via PowerShell
docker exec %SAMBA_CONTAINER% ldapsearch -LLL -x -H ldap://127.0.0.1:389 -D "CN=Administrator,CN=Users,DC=tstf,DC=axacolpatria,DC=co" -w "P@ssw0rd2024" -b "OU=Grupos,DC=tstf,DC=axacolpatria,DC=co" -s one "(objectClass=group)" cn > "%TEMP%\ad-groups-raw.txt" 2>NUL

powershell -Command "Get-Content '%TEMP%\ad-groups-raw.txt' | Where-Object { $_ -match '^cn: ' } | ForEach-Object { ($_ -replace '^cn: ','').Trim() }" > "%TEMP%\ad-groups.txt"

REM Count groups
set GROUP_COUNT=0
for /f "usebackq tokens=*" %%g in ("%TEMP%\ad-groups.txt") do (
    set /a GROUP_COUNT+=1
)

if !GROUP_COUNT! EQU 0 (
    echo    ERROR: No groups found in OU=Grupos.
    echo    Verify Samba AD is running and groups exist.
    del "%TEMP%\ad-groups-raw.txt" >NUL 2>&1
    del "%TEMP%\ad-groups.txt" >NUL 2>&1
    exit /b 1
)
...
set SQL_FILE=%TEMP%\seed-scim-groups-ad.sql
echo.
