#!/bin/bash
# =============================================================
# Seed SCIM metadata for AD groups in WSO2 IS H2 database
#
# DYNAMIC: Discovers all groups in OU=Grupos from Samba AD
# via ldapsearch, then seeds SCIM metadata for each one.
# Can be re-run after adding/removing groups in AD.
#
...
# Samba AD connection
SAMBA_CONTAINER="samba-ad"
SAMBA_ADMIN_DN="CN=Administrator,CN=Users,DC=tstf,DC=axacolpatria,DC=co"
SAMBA_ADMIN_PASS="P@ssw0rd2024"
GROUPS_BASE_DN="OU=Grupos,DC=tstf,DC=axacolpatria,DC=co"
DOMAIN="TSTF"
ID_BASE=1001
...
# Wait for WSO2 IS to be ready (ensures DB schema exists and groups discovered from AD)
echo ""
...
# Discover groups from Samba AD
echo ""
echo ">> Discovering groups in ${GROUPS_BASE_DN}..."
GROUPS=$(docker exec ${SAMBA_CONTAINER} ldapsearch -LLL -x -H ldap://127.0.0.1:389 \
  -D "${SAMBA_ADMIN_DN}" -w "${SAMBA_ADMIN_PASS}" \
  -b "${GROUPS_BASE_DN}" -s one "(objectClass=group)" cn \
  2>/dev/null | grep "^cn: " | sed 's/^cn: //' | tr -d '\r')

if [ -z "$GROUPS" ]; then
  echo "   ERROR: No groups found in ${GROUPS_BASE_DN}"
  echo "   Verify Samba AD is running and groups exist in the OU."
  exit 1
fi
